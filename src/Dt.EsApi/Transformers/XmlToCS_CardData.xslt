...
<xsl:template match="namespace"><xsl:text>namespace </xsl:text>
...
<xsl:template match="struct">[GeneratedCode("xslt-proc", "undefined")]
    ...
    {
        #region Fields
<xsl:for-each select="field"><xsl:if test="@fieldname!=''">
        ///&lt;summary&gt; <xsl:value-of select="@fieldname"/> &lt;/summary&gt;</xsl:if>
...
<xsl:for-each select="collection_field"><xsl:if test="@fieldname!=''">
...
<xsl:for-each select="collection_field">
        ...
        public override bool Equals(object? other)
        {
            ...
            <xsl:choose>
              <xsl:when test="@isjson='true'">
                JsonComparer.IsParsedAndEquals(<xsl:value-of select="@name"/>, casted.<xsl:value-of select="@name"/>)<xsl:if test="not (position() = last())"> &amp;&amp;</xsl:if><xsl:if test="position() = last()">;</xsl:if>
              </xsl:when>
              <xsl:otherwise>
                ...
                </xsl:call-template>&gt;.Default.Equals(<xsl:value-of select="@name"/>, casted.<xsl:value-of select="@name"/>)<xsl:if test="not (position() = last())"> &amp;&amp;</xsl:if><xsl:if test="position() = last()">;</xsl:if>
              </xsl:otherwise>
...
<xsl:call-template name="operatorsEqNotEq"><xsl:with-param name="typeName"><xsl:value-of select="@name"/></xsl:with-param></xsl:call-template>
        ...
        {
            var stringBuilder = new StringBuilder();<xsl:for-each select="field">
